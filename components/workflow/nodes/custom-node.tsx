...
export function CustomNode({ data, id, selected }: NodeProps<NodeData>) {
  ...
  const { nodeExecutionState, nodeResults, isWorkflowRunning } = useWorkflowStore();
  ...
  const handleCheckRecentFiles = () => {
    if (data.type === 'telegram-receive' && isWorkflowRunning && nodeResult?.checkRecentFiles) {
  ...
  return (
    <motion.div
      ...
      className={`w-56 rounded-lg border shadow-sm bg-card overflow-hidden ${selected ? 'ring-2 ring-primary' : ''
        }`}
    >
      <div
        className={`p-3 font-medium border-b flex items-center gap-2 ${data.type.includes('trigger') || data.type === 'telegram-receive' ? 'bg-mauve/20' :
          data.type.includes('action') || data.type === 'arweave-upload' ? 'bg-sapphire/20' :
            'bg-peach/20'
          }`}
      >
      ...
      <div className="p-3 text-xs text-muted-foreground">
        ...
        {data.type === 'telegram-receive' && nodeState === 'running' && (
          <div className="mt-2">
            <Button
              variant="outline"
              size="sm"
