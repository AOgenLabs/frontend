...
export interface Edge<T = any> extends ReactFlowEdge<T> {
  id: string;
  type?: string;
  source: string;
  target: string;
...
export interface NodeChange extends ReactFlowNodeChange { }
export interface EdgeChange extends ReactFlowEdgeChange { }
...
type WorkflowState = {
  ...
  nodeExecutionState: Record<string, 'pending' | 'running' | 'success' | 'error'>;
...
export const useWorkflowStore = create<WorkflowState>((set, get) => ({
  ...
  updateNodeData: (nodeId, data) => {
    try {
      ...
      set((state) => {
        ...
        if (data.config) {
          updatedData.config = { ...node.data.config, ...data.config };
        ...
        console.log(`Node ${nodeId} updated successfully:`, updatedNode?.data);
  ...
  stopWorkflow: async () => {
    try {
      ...
      const telegramNodes = nodes.filter(node => node.data.type === 'telegram-receive');

      if (telegramNodes.length > 0) {
        console.log(`Stopping ${telegramNodes.length} Telegram nodes...`);

        for (const node of telegramNodes) {
          console.log(`Stopping Telegram node: ${node.data.label} (${node.id})`);

          if (nodeResults[node.id]?.stop) {
            try {
              await nodeResults[node.id].stop();
              console.log(`Successfully stopped Telegram node: ${node.id}`);
            } catch (error) {
              console.error(`Error stopping Telegram node ${node.id}:`, error);
            }
          } else {
            console.warn(`No stop method found for Telegram node: ${node.id}`);
          }
        }
      }
      ...
      switch (node.data.type) {
        case 'telegram-receive': {
          try {
            ...
            const telegramHandler = await nodeExecutors.executeTelegramReceive(node.data.config);
            ...
            console.log('Telegram node is now waiting for changes in recent files...');

            // Also add a method to manually check for recent files
            const checkRecentFiles = async () => {
              console.log('Manually checking for recent files...');
              if (get().isWorkflowRunning) {
                const recentFiles = await telegramHandler.processRecentFiles();
                if (recentFiles.length > 0) {
                  console.log(`Found new file, passing to connected nodes...`);
                  ...
                  console.log(`Found ${outgoingEdges.length} outgoing edges from Telegram node`);

                  // For the latest file, execute the next nodes with the file data as input
            ...
            telegramHandler.startPolling(async (fileData) => {
        ...
        case 'arweave-upload': {
          try {
            if (!inputData) {
              console.error('No file data provided to Arweave upload node');
        ...
        default:
          console.warn(`No executor implemented for node type: ${node.data.type}`);
